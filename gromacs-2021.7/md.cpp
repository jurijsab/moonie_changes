...
void gmx::LegacySimulator::do_md()
{
    ...
    while (!bLastStep)
    {
        ...
        else
        {
            ...
            /*for (unsigned neki=0; neki < state->x.size(); neki++){
            	//if(neki != 4 && neki != 6 && neki != 8 && neki != 14 && neki != 16) continue;
            	printf("GROMACS :: md.cpp :: before SHAKE :: neki: %i, pos: %f %f %f, vel: %f %f %f \n", neki, state->x[neki][0], state->x[neki][1], state->x[neki][2], state->v[neki][0], state->v[neki][1], state->v[neki][2]);
            }*/
            ...
            if(plumedswitch){
              ...
              plumed_cmd(plumedmain,"setVelocities",&state->v[0][0]); // changes for MOONIE 04.10.2024
              ...
              //TO DO SHAKE // changes for MOONIE 04.10.2024
              
              //printf("GROMACS::before performCalc at SHAKE \n");
              
              plumed_cmd(plumedmain,"performCalc",nullptr);
              
              //printf("GROMACS::after performCalc at SHAKE \n");
            }
            
            /*for (unsigned neki=0; neki < state->x.size(); neki++){
            	//if(neki != 4 && neki != 6 && neki != 8 && neki != 14 && neki != 16) continue;
            	printf("GROMACS :: md.cpp :: after SHAKE :: neki: %i, pos: %f %f %f, vel: %f %f %f \n", neki, state->x[neki][0], state->x[neki][1], state->x[neki][2], state->v[neki][0], state->v[neki][1], state->v[neki][2]);
            }*/
        ...
        /*  ############### START FIRST UPDATE HALF-STEP FOR VV METHODS############### */
        {
			...
			/*for (unsigned neki=0; neki < state->x.size(); neki++){
            	if(neki != 0 || neki != 18) continue;
            	printf("GROMACS :: md.cpp :: before update_coords etrtVELOCITY1 (VV second part) :: neki: %i, pos: %f %f %f, vel: %f %f %f \n", neki, state->x[neki][0], state->x[neki][1], state->x[neki][2], state->v[neki][0], state->v[neki][1], state->v[neki][2]);
            }*/
            ...
            /*for (unsigned neki=0; neki < state->x.size(); neki++){
            	if(neki != 0 || neki != 18) continue;
            	printf("GROMACS :: md.cpp :: after update_coords etrtVELOCITY1 (VV second part) :: neki: %i, pos: %f %f %f, vel: %f %f %f \n", neki, state->x[neki][0], state->x[neki][1], state->x[neki][2], state->v[neki][0], state->v[neki][1], state->v[neki][2]);
            }*/
        ...
        // changes for MOONIE 04.10.2024
        //printf("GROMACS::before performCalc at RATTLE \n");
            
        /*for (unsigned neki=0; neki < state->x.size(); neki++){
          	//if(neki != 4 && neki != 6 && neki != 8 && neki != 14 && neki != 16) continue;
          	printf("GROMACS :: md.cpp :: before RATTLE :: neki: %i, pos: %f %f %f, vel: %f %f %f \n", neki, state->x[neki][0], state->x[neki][1], state->x[neki][2], state->v[neki][0], state->v[neki][1], state->v[neki][2]);
        }*/
            	  
        if(plumedswitch){
          	//long int llstep=step; plumed_cmd(plumedmain,"setStepLong",&llstep);
           	//plumed_cmd(plumedmain,"setVelocities",&state_->v[0][0]); // changes for MOONIE 01.10.2024
           	plumed_cmd(plumedmain,"performCalc",nullptr);
		}            
        ...
        /*for (unsigned neki=0; neki < state->x.size(); neki++){
           	//if(neki != 4 && neki != 6 && neki != 8 && neki != 14 && neki != 16) continue;
           	printf("GROMACS :: md.cpp :: after RATTLE :: neki: %i, pos: %f %f %f, vel: %f %f %f \n", neki, state->x[neki][0], state->x[neki][1], state->x[neki][2], state->v[neki][0], state->v[neki][1], state->v[neki][2]);
        }*/
            	  
        //printf("GROMACS::after performCalc at RATTLE \n");
            
        // TO DO RATTLE // changes for MOONIE 04.10.2024
        ...
        if (EI_VV(ir->eI))
        {
            ...
            /*for (unsigned neki=0; neki < state->x.size(); neki++){
            	if(neki != 0 || neki != 18) continue;
            	printf("GROMACS :: md.cpp :: before update_coords etrtVELOCITY2 (VV first part) :: neki: %i, pos: %f %f %f, vel: %f %f %f \n", neki, state->x[neki][0], state->x[neki][1], state->x[neki][2], state->v[neki][0], state->v[neki][1], state->v[neki][2]);
            }*/
            ...
            /*for (unsigned neki=0; neki < state->x.size(); neki++){
            	if(neki != 0 || neki != 18) continue;
            	printf("GROMACS :: md.cpp :: after update_coords etrtVELOCITY2 (VV first part) :: neki: %i, pos: %f %f %f, vel: %f %f %f \n", neki, state->x[neki][0], state->x[neki][1], state->x[neki][2], state->v[neki][0], state->v[neki][1], state->v[neki][2]);
            }*/
        }
        ...
        else
        {
            ...
            /*for (unsigned neki=0; neki < state->x.size(); neki++){
            	if(neki != 0 || neki != 18) continue;
            	printf("GROMACS :: md.cpp :: before update_coords etrtPOSITION (VV first part) :: neki: %i, pos: %f %f %f, vel: %f %f %f \n", neki, state->x[neki][0], state->x[neki][1], state->x[neki][2], state->v[neki][0], state->v[neki][1], state->v[neki][2]);
            }*/
            ...
            /*for (unsigned neki=0; neki < state->x.size(); neki++){
            	if(neki != 0 || neki != 18) continue;
            	printf("GROMACS :: md.cpp :: after update_coords etrtPOSITION (VV first part) :: neki: %i, pos: %f %f %f, vel: %f %f %f \n", neki, state->x[neki][0], state->x[neki][1], state->x[neki][2], state->v[neki][0], state->v[neki][1], state->v[neki][2]);
            }*/
            ...
            /*for (unsigned neki=0; neki < state->x.size(); neki++){
            	if(neki != 0 || neki != 18) continue;
            	printf("GROMACS :: md.cpp :: after finish_update :: neki: %i, pos: %f %f %f, vel: %f %f %f \n", neki, state->x[neki][0], state->x[neki][1], state->x[neki][2], state->v[neki][0], state->v[neki][1], state->v[neki][2]);
            }*/
        }
