...
Atoms::Atoms(PlumedMain&plumed):
  ...
  velocitiesHaveBeenSet(0), ///WHY NOT 0???
...
/// changes for MOONIE 19.07.2024
void Atoms::setVelocities(const TypesafePtr & p) {
...
/// changes for MOONIE 19.07.2024
void Atoms::setVelocities(const TypesafePtr & p,int i) {
  plumed_massert( dataCanBeSet,"setVelocities must be called after setStep in MD code interface");
  plumed_massert( p || gatindex.size()==0, "NULL velocities pointer with non-zero local atoms");
  mdatoms->setp(p,i); velocitiesHaveBeenSet++;
}
...
void Atoms::share(const std::vector<AtomNumber>& unique) {
  /// changes for MOONIE 19.07.2024
  plumed_assert( velocitiesHaveBeenSet==3 && positionsHaveBeenSet==3 && massesHaveBeenSet );
  ...
  if(!(int(gatindex.size())==natoms && shuffledAtoms==0)) {
    ...
    /// changes for MOONIE 19.07.2024
    mdatoms->getVelocities(unique,uniq_index,velocities);
...
// how many double per atom should be scattered:
  ...
  if(dd && shuffledAtoms>0) {
    if(dd.async) {
      ...
      /// changes for MOONIE 19.07.2024
      for(unsigned i=0; i<dd.mpi_request_velocities.size(); i++) dd.mpi_request_velocities[i].wait();
    ...
    for(const auto & p : unique) {
      ...
      /// changes for MOONIE 19.07.2024
      dd.velocitiesToBeSent[3*count+0]=velocities[p.index()][0];
      dd.velocitiesToBeSent[3*count+1]=velocities[p.index()][1];
      dd.velocitiesToBeSent[3*count+2]=velocities[p.index()][2];
    ...
    if(dd.async) {
      ...
      /// changes for MOONIE 19.07.2024
      dd.mpi_request_velocities.resize(dd.Get_size());
      for(int i=0; i<dd.Get_size(); i++) {
        ...
        /// changes for MOONIE 19.07.2024
        dd.mpi_request_velocities[i]=dd.Isend(&dd.velocitiesToBeSent[0],3*count,i,668); // 668 or 667
      }
    } else {
      ...
      /// changes for MOONIE 19.07.2024
      std::vector<int> counts3(n);
      std::vector<int> displ3(n);
      ...
      /// changes for MOONIE 19.07.2024
      for(int i=0; i<n; ++i) counts3[i]=counts[i]*3;
      for(int i=0; i<n; ++i) displ3[i]=displ[i]*3;
      ...
      /// changes for MOONIE 19.07.2024
      dd.Allgatherv(&dd.velocitiesToBeSent[0],3*count,&dd.velocitiesToBeReceived[0],&counts3[0],&displ3[0]);
      ...
      for(int i=0; i<tot; i++) {
        ...
        /// changes for MOONIE 19.07.2024
        velocities[dd.indexToBeReceived[i]][0]=dd.velocitiesToBeReceived[3*i+0];
        velocities[dd.indexToBeReceived[i]][1]=dd.velocitiesToBeReceived[3*i+1];
        velocities[dd.indexToBeReceived[i]][2]=dd.velocitiesToBeReceived[3*i+2];
...
// receive toBeReceived
    if(asyncSent) {
      ...
      for(int i=0; i<dd.Get_size(); i++) {
        ...
        /// changes for MOONIE 19.07.2024
        dd.Recv(&dd.velocitiesToBeReceived[3*count],dd.velocitiesToBeReceived.size()-3*count,i,668);
...
void Atoms::updateVelocities()
{
  
  plumed_assert( velocitiesHaveBeenSet==3 );
  if(!unique_serial && int(gatindex.size())==natoms && shuffledAtoms==0) mdatoms->updateVelocities(gatindex,velocities);  
  else mdatoms->updateVelocities(unique,uniq_index,velocities);
  
}
/// changes for MOONIE 24.07.2024
void Atoms::updatePositions()
{

  plumed_assert( positionsHaveBeenSet==3 );
  if(!unique_serial && int(gatindex.size())==natoms && shuffledAtoms==0) mdatoms->updatePositions(gatindex,positions);
  else mdatoms->updatePositions(unique,uniq_index,positions);  
...
// avoid errors when doing boundary check
    ...
    /// changes for MOONIE 19.07.2024
    dd.velocitiesToBeSent.resize(n*3,0.0);
    dd.velocitiesToBeReceived.resize(natoms*3,0.0);
...
void Atoms::writeBinary(std::ostream&o)const {
  ...
  /// changes for MOONIE 19.07.2024
  o.write(reinterpret_cast<const char*>(&velocities[0][0]),natoms*3*sizeof(double));
...
void Atoms::readBinary(std::istream&i) {
  ...
  /// changes for MOONIE 19.07.2024
  i.read(reinterpret_cast<char*>(&velocities[0][0]),natoms*3*sizeof(double));
