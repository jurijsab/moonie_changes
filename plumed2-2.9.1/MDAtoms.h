...
/// Retrieve local positions.
  virtual void getLocalPositions(std::vector<Vector>&v)const=0;
...
/// changes for MOONIE 19.07.2024
  // The operation is done in such a way that p[index[i]] is equal to the velocities of atom i
  virtual void getVelocities(const std::vector<int>&index,std::vector<Vector>&velocities)const=0;
  // Retrieve all atom velocities from index i to index j.
  virtual void getVelocities(unsigned i,unsigned j,std::vector<Vector>&velocities)const=0;
  // Retrieve all atom velocities from atom indices and local indices.
  virtual void getVelocities(const std::vector<AtomNumber>&index,const std::vector<unsigned>&i,std::vector<Vector>&velocities)const=0;
  
  virtual void updateVelocities(const std::vector<int>&index,const std::vector<Vector>&v)=0;
  virtual void updatePositions(const std::vector<int>&index,const std::vector<Vector>&pos)=0; 
  /// Set a pointer to the velocities array in the MD code
  virtual void setv(const TypesafePtr & p)=0;
  /// Set a pointer to the position array in the MD code
  virtual void setv(const TypesafePtr & p,int i)=0;
  
/// changes for MOONIE 24.07.2024  
  virtual void updateVelocities(const std::vector<AtomNumber>&index,const std::vector<unsigned>&i,const std::vector<Vector>&v)=0;
  virtual void updatePositions(const std::vector<AtomNumber>&index,const std::vector<unsigned>&i,const std::vector<Vector>&pos)=0;
