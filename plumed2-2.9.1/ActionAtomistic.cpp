...
void ActionAtomistic::retrieveAtoms() {
  ...
  /// changes for MOONIE 19.07.2024
  const std::vector<Vector> & v(atoms.velocities);
...
/// changes for MOONIE 19.07.2024
void ActionAtomistic::applyPositions() {
  std::vector<Vector>& p(atoms.positions);
  for(unsigned j=0; j<indexes.size(); j++) p[indexes[j].index()]=positions[j];
}

void ActionAtomistic::applyVelocities() {
  std::vector<Vector>& v(atoms.velocities);
  for(unsigned j=0; j<indexes.size(); j++) v[indexes[j].index()]=velocities[j];
}

void ActionAtomistic::updatePositions(){
  atoms.updatePositions();  
}

void ActionAtomistic::updateVelocities(){
  atoms.updateVelocities();  
}
