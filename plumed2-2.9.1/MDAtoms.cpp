  ...
  public MDAtomsBase
{
  ...
  T scalevel=1.0; // factor to scale velocities /// changes for MOONIE 19.07.2024
  TypesafePtr m;
  TypesafePtr c;
  TypesafePtr p;
  TypesafePtr px,py,pz;
  /// changes for MOONIE 19.07.2024 from here
  TypesafePtr v;
  TypesafePtr vx,vy,vz;
  /// changes for MOONIE 19.07.2024 to here
  TypesafePtr f;
...
public:
  ...
  void getVelocities(const std::vector<int>&index,std::vector<Vector>&velocities) const override;
  void getVelocities(const std::vector<AtomNumber>&index,const std::vector<unsigned>&i,std::vector<Vector>&velocities) const override;
  void getVelocities(unsigned j,unsigned k,std::vector<Vector>&velocities) const override;
  
  void getLocalVelocities(std::vector<Vector>&v) const override;
  
  void updateVelocities(const std::vector<int>&index,const std::vector<Vector>&v) override;
  void updatePositions(const std::vector<int>&index,const std::vector<Vector>&pos) override;
  
  /// changes for MOONIE 19.07.2024
  void updateVelocities(const std::vector<AtomNumber>&index,const std::vector<unsigned>&i,const std::vector<Vector>&v) override;
  void updatePositions(const std::vector<AtomNumber>&index,const std::vector<unsigned>&i,const std::vector<Vector>&pos) override;
  
  void setv(const TypesafePtr & p) override;
  void setv(const TypesafePtr & p,int i) override;
};
...
void MDAtomsTyped<T>::setUnits(const Units& units,const Units& MDUnits) {
  ...
  double tscale=units.getTime()/MDUnits.getTime(); /// changes for MOONIE 19.07.2024
...
// scalef and scalev are used to convert plumed to MD
  ...
  scalec=1.0/cscale;
  scalem=1.0/mscale;
  scalevel=tscale/lscale; /// changes for MOONIE 19.07.2024
}
...
/// changes for MOONIE 19.07.2024
template <class T>
void MDAtomsTyped<T>::getVelocities(const std::vector<int>&index,std::vector<Vector>&velocities)const {
  unsigned stride;
  const T* vvx;
  const T* vvy;
  const T* vvz;
  getPointers(v,vx,vy,vz,index.size(),vvx,vvy,vvz,stride);
  plumed_assert(index.size()==0 || (vvx && vvy && vvz));
// cannot be parallelized with omp because access to positions is not ordered
  for(unsigned i=0; i<index.size(); ++i) {
    velocities[index[i]][0]=vvx[stride*i]*scalevel;
    velocities[index[i]][1]=vvy[stride*i]*scalevel;
    velocities[index[i]][2]=vvz[stride*i]*scalevel;
  }
}
...
/// changes for MOONIE 19.07.2024
template <class T>
void MDAtomsTyped<T>::getVelocities(const std::vector<AtomNumber>&index,const std::vector<unsigned>&i, std::vector<Vector>&velocities)const {
  unsigned stride;
  const T* vvx;
  const T* vvy;
  const T* vvz;
...
// bounds are only checked in debug mode since they require this extra step that is potentially expensive
  ...
  const unsigned maxel=0;
#endif
  getPointers(v,vx,vy,vz,maxel,vvx,vvy,vvz,stride);
  plumed_assert(index.size()==0 || (vvx && vvy && vvz));
// cannot be parallelized with omp because access to positions is not ordered
  unsigned k=0;
  for(const auto & v : index) {
    velocities[v.index()][0]=vvx[stride*i[k]]*scalevel;
    velocities[v.index()][1]=vvy[stride*i[k]]*scalevel;
    velocities[v.index()][2]=vvz[stride*i[k]]*scalevel;
    k++;
  }
}
...
/// changes for MOONIE 19.07.2024
template <class T>
void MDAtomsTyped<T>::getVelocities(unsigned j,unsigned k,std::vector<Vector>&velocities)const {
  unsigned stride;
  const T* vvx;
  const T* vvy;
  const T* vvz;
  getPointers(v,vx,vy,vz,k,vvx,vvy,vvz,stride);
  plumed_assert(k==j || (vvx && vvy && vvz));
  #pragma omp parallel for num_threads(OpenMP::getGoodNumThreads(&velocities[j],(k-j)))
  for(unsigned i=j; i<k; ++i) {
    velocities[i][0]=vvx[stride*i]*scalevel;
    velocities[i][1]=vvy[stride*i]*scalevel;
    velocities[i][2]=vvz[stride*i]*scalevel;
  }
}
...
/// changes for MOONIE 19.07.2024
template <class T>
void MDAtomsTyped<T>::getLocalVelocities(std::vector<Vector>&velocities)const {
  unsigned stride;
  const T* vvx;
  const T* vvy;
  const T* vvz;
  getPointers(v,vx,vy,vz,velocities.size(),vvx,vvy,vvz,stride);
  plumed_assert(velocities.size()==0 || (vvx && vvy && vvz));
...
/// changes for MOONIE 19.07.2024
template <class T>
void MDAtomsTyped<T>::updatePositions(const std::vector<int>&index,const std::vector<Vector>&positions) {
  unsigned stride;
  T* ppx;
  T* ppy;
  T* ppz;
  getPointers(p,px,py,pz,index.size(),ppx,ppy,ppz,stride);
  plumed_assert(index.size()==0 || (ppx && ppy && ppz));
  #pragma omp parallel for num_threads(OpenMP::getGoodNumThreads(ppx,stride*index.size()))
  for(unsigned i=0; i<index.size(); ++i) {
    ppx[stride*i]=scalep*T(positions[index[i]][0]);
    ppy[stride*i]=scalep*T(positions[index[i]][1]);
    ppz[stride*i]=scalep*T(positions[index[i]][2]);
  }
}

/// changes for MOONIE 24.07.2024
template <class T>
void MDAtomsTyped<T>::updatePositions(const std::vector<AtomNumber>&index,const std::vector<unsigned>&i,const std::vector<Vector>&positions) {
  unsigned stride;
  T* ppx;
  T* ppy;
  T* ppz;
...
// bounds are only checked in debug mode since they require this extra step that is potentially expensive
  ...
  plumed_assert(index.size()==0 || (ppx && ppy && ppz));
  unsigned k=0;
  for(const auto & z : index) {
    ppx[stride*i[k]]=scalep*T(positions[z.index()][0]);
    ppy[stride*i[k]]=scalep*T(positions[z.index()][1]);
    ppz[stride*i[k]]=scalep*T(positions[z.index()][2]);
    k++;
  }
}
...
/// changes for MOONIE 19.07.2024
template <class T>
void MDAtomsTyped<T>::updateVelocities(const std::vector<int>&index,const std::vector<Vector>&velocities) {
  unsigned stride;
  T* vvx;
  T* vvy;
  T* vvz;
  getPointers(v,vx,vy,vz,index.size(),vvx,vvy,vvz,stride);
  plumed_assert(index.size()==0 || (vvx && vvy && vvz));
  #pragma omp parallel for num_threads(OpenMP::getGoodNumThreads(vvx,stride*index.size()))
  for(unsigned i=0; i<index.size(); ++i) {
    vvx[stride*i]=scalevel*T(velocities[index[i]][0]);
    vvy[stride*i]=scalevel*T(velocities[index[i]][1]);
    vvz[stride*i]=scalevel*T(velocities[index[i]][2]);
  }
}

/// changes for MOONIE 24.07.2024
template <class T>
void MDAtomsTyped<T>::updateVelocities(const std::vector<AtomNumber>&index,const std::vector<unsigned>&i,const std::vector<Vector>&velocities) {
  unsigned stride;
  T* vvx;
  T* vvy;
  T* vvz;
...
// bounds are only checked in debug mode since they require this extra step that is potentially expensive
  ...
  unsigned k=0;
  for(const auto & z : index) {
    vvx[stride*i[k]]=scalevel*T(velocities[z.index()][0]);
    vvy[stride*i[k]]=scalevel*T(velocities[z.index()][1]);
    vvz[stride*i[k]]=scalevel*T(velocities[z.index()][2]);
    //vvz[stride*i[k]]+=scalevel*T(velocities[z.index()][2]); JS and SD found that they were evidently drunk on 24.07.2024
    k++;
  }
}
...
/// changes for MOONIE 19.07.2024
template <class T>
void MDAtomsTyped<T>::setv(const TypesafePtr & vv) {
  vv.get<const T*>(); // just check type and discard pointer
  v=vv.copy();
  vx=TypesafePtr();
  vy=TypesafePtr();
  vz=TypesafePtr();
...
/// changes for MOONIE 19.07.2024
template <class T>
void MDAtomsTyped<T>::setv(const TypesafePtr & vv,int i) {
  v=TypesafePtr();
  vv.get<const T*>(); // just check type and discard pointer
  if(i==0)vx=vv.copy();
  if(i==1)vy=vv.copy();
